...
namespace flow
{
    ...
    static string inc_path;
    static bool debug = false;
    static std::set<string> nonliners =  {"{" /*"}" REMOVED BECAUSE OF ARRAY IMPLEMENTATION*/};;
    bool compile(string fc, string outputcpp,bool outputex,string outputexe, bool runCompileAfter);
    ...
    public:
        ...
        inline static string readFileIntoString(string path, std::set<string> s)
        {
            ...
            {
                if (debug) cout << "an unexpected error has occured while trying to open the file." << endl;
                return "";
            }
        }
        ...
        inline static string endliner(string prev, std::set<string> s)
        {
            ...
            while (getline(f, line))
            {
                ...
                for (auto a : s)
                {
                    if (debug) cout << a << "has?:" << has_suffix(line, a) << endl;
                ...
                if (debug) cout << needs << endl;
                if (needs)
                {
                    if (debug) cout << "appending" + (line + ";") << endl;
        ...
        inline static bool writeToFile(string path, string cont, string placeholder = "")
        {
            ...
            {
                if (debug) cout << "exce:" << e.what() << endl;
                return false;
            }
        }
    };
